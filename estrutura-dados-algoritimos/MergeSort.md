...
> Se quero **só encontrar** o meio, `slow` e `fast` começam do mesmo ponto.
> Mas se quero **cortar** a lista (como no merge sort),
...
1. **Começo simples**: se a lista tem 0 ou 1 nó, ela já está ordenada. Retorna direto.
2. **Divisão**: usa `slow` e `fast` pra achar o meio. Depois separa com `meio.proximo = null`.
3. **Recursão**: chama `mergeSort()` pra cada metade.
4. **Combinação**: junta os resultados com o método `merge()`.
